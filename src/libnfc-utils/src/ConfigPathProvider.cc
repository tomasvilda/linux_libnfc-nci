...
string ConfigPathProvider::getFilePath(FileType type) {
    ...
    case VENDOR_NFC_CONFIG: {
        string path = std::string(CONFIG_PATH) + "libnfc-nxp.conf";
        addEnvPathIfAvailable(path);
        return path;
    } break;
    case VENDOR_ESE_CONFIG: {
        string path = std::string(CONFIG_PATH) + "libese-nxp.conf";
        addEnvPathIfAvailable(path);
        return path;
    } break;
    case SYSTEM_CONFIG: {
    ...
    case RF_CONFIG: {
        string path = std::string(CONFIG_PATH) + "libnfc-nxp_RF.conf";
        addEnvPathIfAvailable(path);
        return path;
    } break;
    case TRANSIT_CONFIG: {
        string path = std::string(CONFIG_PATH) + "libnfc-nxpTransit.conf";
        addEnvPathIfAvailable(path);
        return path;
    } break;
    case NFASTORAGE_BIN: {
    ...
    case FIRMWARE_LIB: {
        string path = std::string(CONFIG_PATH) + "libsn100u_fw.dll";
        addEnvPathIfAvailable(path);
        return path;
    } break;
    case CONFIG_TIMESTAMP: {
        string path = std::string(CONFIG_PATH) + "libnfc-nxpConfigState.bin";
        addEnvPathIfAvailable(path);
        return path;
    } break;
    case RF_CONFIG_TIMESTAMP: {
        string path = std::string(CONFIG_PATH) + "libnfc-nxpRFConfigState.bin";
        addEnvPathIfAvailable(path);
        return path;
    } break;
    case TRANSIT_CONFIG_TIMESTAMP: {
        string path = std::string(CONFIG_PATH) + "libnfc-nxpTransitConfigState.bin";
        addEnvPathIfAvailable(path);
        return path;
    } break;
