...
static bool_t phNxpNciHal_is_config_probe_rsp(const uint8_t* p_rsp_data,
        uint16_t rsp_len);
...
{
    NFCSTATUS status = NFCSTATUS_FAILED;
    bool_t is_config_probe_rsp = FALSE;
    UNUSED(pContext);
        ...
        uint16_t rsp_len)
{
    ...
    if ((p_rsp_data[0] & NXP_NCI_MT_MASK) != NXP_NCI_MT_RSP
            || (p_rsp_data[1] & NXP_NCI_OID_MASK) != NXP_NCI_MSG_CORE_GET_CONFIG)
    {
        return FALSE;
    }
    ...
    return (p_rsp_data[4] == 0x01 && p_rsp_data[5] == 0xA0
            && p_rsp_data[6] == 0x07 && p_rsp_data[7] == 0x01) ? TRUE : FALSE;
}
...
int phNxpNciHal_isConfigured(void)
{
    ...
    param_val = (rsp_len >= 9
            ...
            ? nxpncihal_ctrl.config_probe_rsp_data[8] : 0xFF;
    nxpncihal_ctrl.config_probe_rsp_len = 0;
    CONCURRENCY_UNLOCK();
